								...
								<?php foreach ($order_list_tomeng as $item):?>
									<tr>
										...
										<td id="status<?php echo $item['order_id'];?>">
											<div style='color:#ff2821'>未支付</div>
										</td>

										<td><div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<a href="<?php url('order/detail/'.$item['order_id'])?>">查看</a>
													<a href="javascript:void(0);" onclick="request_url = base_url + 'order/delete';if(confirm('确认删除？')){$.post(request_url,{order_id:<?php echo $item['order_id'];?>},function(data) {alert(data);location.reload(true);})}">删除</a>
												</div>
											</div></td>
									</tr>
								<?php endforeach;?>
